...
dossiers_fictifs = [generate_fake_data() for _ in range(500)]
...
def calculer_visa(row):
    # Règles bloquantes globales
    if row['Fournisseur blacklisté'] == 'Oui':
        return 'Non'
    if row['Situation fiscale'] == 'Non conforme':
        return 'Non'
    if row['Notation interne'] < 12:
        return 'Non'
    if row['Expérience fournisseur'] < 2 and row['Nombre de projets similaires'] < 2:
        return 'Non'
    
    # Règles spécifiques selon Typologie
    if row['Typologie du marché'] == 'Fournitures':
        if row['Montant du contrat'] > 7000000 and row['Garantie'] == 'Aucune':
            return 'Non'
        if row['Délai de réalisation'] > 6:
            return 'Non'

    elif row['Typologie du marché'] == 'Services':
        if row['Montant du contrat'] > 8000000:
            return 'Non'
        if row['Garantie'] == 'Aucune' and row['Montant du contrat'] > 6000000:
            return 'Non'

    elif row['Typologie du marché'] == 'Travaux':
        if row['Montant du contrat'] > 9000000 and row['Garantie'] != 'Caution':
            return 'Non'
